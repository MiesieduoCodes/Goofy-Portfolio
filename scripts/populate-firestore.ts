// Firebase Firestore data population script
// Run with: npm run populate:firestore
...
async function addData(collectionName: string, data: any) {
  try {
    ...
    await setDoc(docRef, { ...data, createdAt: Date.now(), id: docRef.id });
    console.log(`✅ Added ${collectionName}:`, data.name || data.title || data.role);
    return docRef.id;
  } catch (error) {
    console.error(`❌ Failed to add ${collectionName}:`, error);
    throw error;
  }
}
...
async function populateFirestoreDatabase() {
  ...
  try {
    ...
    if (!isConnected) {
      console.log("❌ Firestore database is not connected or accessible.");
      console.log("📋 Please check:");
      console.log("   1. Firestore is enabled in your project");
      console.log("   2. Firestore rules allow read/write access");
      console.log("   3. Network connectivity is working");
      console.log("\n🔗 Firebase Console: https://console.firebase.google.com/project/miesieduocodes/firestore");
      return;
    }
    ...
    console.log("\n🎉 Firestore database population completed successfully!");
    console.log("📊 Summary:");
...
export { populateFirestoreDatabase };
